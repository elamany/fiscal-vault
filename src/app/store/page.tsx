...
function ProductCard({ product, isLoggedIn }: { product: Product; isLoggedIn: boolean }) {
  ...
  return (
    <div className="group bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
      <Link href={getProductPath(product.name, product.id)} className="block relative h-48 w-full overflow-hidden bg-gray-100">
        <Image
          src={imageUrl} 
          alt={product.name}
          fill 
      ...
      <div className="p-4">
        ...
        <Link href={getProductPath(product.name, product.id)}>
